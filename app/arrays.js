arraysAnswers = {
  ...
  /**
   * Return the original array with the same items as arr, excluding item
   *
   * @param {Number[]} arr - An array of numbers
   * @param {Number} item - A number to be excluded from the new array
  ...
  duplicates: function duplicates(arr) {
    ...
    for (let i = 0; i < sortedArr.length; i += 1) {
      if (sortedArr[i] === sortedArr[i + 1] && this.count(newArr, sortedArr[i]) === 0) {
  ...
  findAllOccurrences: function findAllOccurrences(arr, target) {
    ...
    arr.forEach((number, index) => {
      if (number === target) this.append(allOccurrences, index);
    });
